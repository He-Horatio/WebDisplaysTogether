...
public class DisplayHandler implements CefDisplayHandler {
    ...
    public void onAddressChange(CefBrowser browser, CefFrame cefFrame, String url) {
        ...
        if (browser != null) {
            ...
            // Server-side (integrated server) browsers: keep ScreenData.url in sync
            net.montoyo.wd.serverbrowser.ServerBrowserManager.onBrowserAddressChange(browser, url);
        }

        // enables a custom pointer lock api
        browser.executeJavaScript(Scripts.POINTER_LOCK, "WDT", 0);
    }
